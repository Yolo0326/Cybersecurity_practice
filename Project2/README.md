...
### 1、水印嵌入
#### 1)初始化与参数设置
首先获取待嵌入水印的图像和水印图像的尺寸信息，进行尺寸合法性校验，确保水印图像尺寸不大于背景图像尺寸按设定块大小划分后的尺寸。同时初始化一些关键参数，如分块大小block_size（默认值为8）、嵌入强度系数alpha，以及用于嵌入时区分水印值（0或1）的两个随机向量k1和k2。
```python
...
```
#### 2)图像分块与DCT变换
将输入的背景图像按照设定的块大小（如8×8）进行分块。对于划分好的每个块，使用cv2.dct函数进行离散余弦变换，将图像从空间域转换到频域，得到对应块的DCT系数矩阵，最终形成一个包含所有分块DCT系数的多维数组，用于后续嵌入水印操作。
```python
    # DCT分块处理
...
```
#### 3)水印嵌入
确保输入的水印图像是经过二值归一化处理的（像素值仅为0或1）。遍历水印图像的每个像素位置（对应到背景图像分块的位置），根据当前水印像素值是0还是1，选择对应的随机向量k1或k2，将水印信息嵌入到对应块的 DCT 系数中。具体是修改每个块的最后一列（特定频率分量）的DCT系数，通过加上与嵌入强度系数alpha、所选随机向量相关的值来实现水印嵌入。
```python
...
### 1、高斯噪声
通过在图像上添加服从高斯分布的随机噪声以模拟传输过程中的随机干扰。
```python
...
```
### 2、椒盐噪声
在图像上随机添加纯白和纯黑像素点，模拟传感器故障或者传输过程中的突发干扰。
```python
...
```
### 3、高通滤波
高通滤波只允许图像中的高频分量通过，可能破坏嵌入在低频区域的水印信息。
```python
...
```
### 4、中值滤波
中值滤波是一种非线性滤波方法，它用像素邻域的中值代替中心像素值，可能破坏水印的精细结构
```python
...
```
### 5、均值滤波
均值滤波是一种线性滤波方法，它用像素邻域的平均值代替中心像素值，可能破坏水印信息
```python
...
根据两幅图片可以看出高通滤波与旋转攻击对水印的影响最大，未能成功提取水印
## 五、参考链接
